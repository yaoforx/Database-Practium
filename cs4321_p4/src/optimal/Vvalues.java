...
public class Vvalues {
    ...
    public void syncrinizeUnionFind(){

        for(UnionFindElement ufe : Selector.unionFind.getUnions()) {
            Double minCost = Double.MAX_VALUE;
            for(Column col : ufe.getColumns()) {
                if(Vvalue.containsKey(col)) {
                    minCost = Vvalue.get(col);
                    updateVvalue(ufe, minCost);
    ...
    private void updateVvalue(UnionFindElement uf, Double minCost) {
        for(Column col : uf.getColumns()) {
           Vvalue.put(col.toString(), minCost);
        }
